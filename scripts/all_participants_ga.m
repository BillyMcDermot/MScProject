...
% initialise results
bws = zeros(length(files), 90);     % best weights (final result of GA)
bfs = zeros(length(files), C);      % best fitness after each competition
mfs = zeros(length(files), C);      % mean fitness after each competition
sfs = zeros(length(files), C);      % standard deviation of fitness after each competition
psis = zeros(length(files), 1);     % psi value when using the best weights to create macro-variable for entire time series

% loop through files and find best macro-variable
